...
function resolveConfig(value: unknown): Config {
  ...
  return {
    ...
    summaryBudgetTokens: finiteInt(
      raw.summaryBudgetTokens,
      DEFAULTS.summaryBudgetTokens,
    ),
    includeSources:
      raw.includeSources === "always" || raw.includeSources === "never"
        ? raw.includeSources
        : "auto",
    ...
    apiTokenEnv:
      typeof raw.apiTokenEnv === "string" ? raw.apiTokenEnv : DEFAULTS.apiTokenEnv,
  };
}
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    const projectRoot =
      process.env.MNEMONIC_VAULT_PROJECT_ROOT ??
      (existsSync(join(sourceRoot, "run.py")) ? sourceRoot : join(homedir(), "mnemonic-vault"));
    const spool = config.autoCapture
      ? new DurableSpool(
          config.spoolPath || join(projectRoot, "data", "spool", "openclaw.jsonl"),
        )
      : undefined;

    const externalSession = (value: {
      sessionKey?: string;
      sessionId?: string;
      runId?: string;
    }): string => value.sessionKey ?? value.sessionId ?? `run-${value.runId ?? "main"}`;
    ...
    const markCaptured = (key: string): boolean => {
    ...
    api.registerTool({
      ...
      parameters: Type.Object({
        ...
        summary_budget_tokens: Type.Optional(
          Type.Integer({ minimum: 100, maximum: 32_000 }),
        ),
        total_context_budget_tokens: Type.Optional(
          Type.Integer({ minimum: 100, maximum: 32_000 }),
        ),
      ...
      async execute(_id, rawParams) {
        ...
        try {
          return toolResult(
            await client.search(params.query, {
    ...
    for (const name of ["memory_get", "memory_open_topic"] as const) {
      api.registerTool({
        name,
    ...
    api.registerTool({
      ...
      parameters: Type.Object({
        ...
        total_token_budget: Type.Optional(
          Type.Integer({ minimum: 300, maximum: 32_000 }),
        ),
      }),
      async execute(_id, rawParams) {
        const params = rawParams as Record<string, any>;
    ...
    api.registerTool({
      ...
      async execute(_id, rawParams) {
        ...
        try {
          return toolResult(
            await client.readTurns(params.session_id, params.from_turn, params.to_turn),
          );
        } catch (error) {
          return errorResult(error);
        }
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        if (!config.autoRecall || !event.prompt.trim()) return;
        try {
    ...
    api.on("agent_end", async (event, ctx) => {
      if (!config.autoCapture || !event.success) return;
